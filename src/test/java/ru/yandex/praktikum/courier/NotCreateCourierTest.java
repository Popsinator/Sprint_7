...
public class NotCreateCourierTest {
    ...
    public void createTwoSimilarCourierOk() {
        ...
        ValidatableResponse responseLoginFirst = courierClient.login(Credentials.from(courier));
    ...
    public void notLoginCourierWithIncorrectLogin() {
        ...
        ValidatableResponse responseLoginIncorrect = courierClient.login(Credentials.from(courier));
    ...
    public void notLoginCourierWithOutPassword() {
        ...
        ValidatableResponse responseLogin = courierClient.login(Credentials.from(courier));
        id = responseLogin.extract().path("id");
        ValidatableResponse responseLoginIncorrect = courierClient.login(CredentialsOnlyLogin.fromOnlyLogin(courier));
        assertTrue(responseCreate.extract().statusCode() == 201
    ...
    public void notLoginCourierWithOutLogin() {
        ...
        ValidatableResponse responseLogin = courierClient.login(Credentials.from(courier));
        id = responseLogin.extract().path("id");
        ValidatableResponse responseLoginIncorrect = courierClient.login(CredentialsOnlyPassword.fromOnlyPassword(courier));
        assertTrue(responseCreate.extract().statusCode() == 201
                && responseLoginIncorrect.extract().statusCode() == 400
                && responseLoginIncorrect.extract().path("message").equals("Недостаточно данных для входа"));
    }
